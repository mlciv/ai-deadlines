...
**To add or update a deadline, see the [Contribution Guide](CONTRIBUTING.md).**
It walks you through it in about two minutes — no programming needed, and you can
even [edit right in your browser](CONTRIBUTING.md#the-easy-way-edit-in-your-browser-no-local-setup)
without setting anything up.

In short: each venue is one file in [`_data/conferences/`](_data/conferences) (a list
of editions, newest first). Add or edit an entry with the required fields — `title`,
`year`, `id`, `link`, `deadline`, `timezone`, `date`, `place`, `sub` — and send a pull
request. The [guide](CONTRIBUTING.md) has a copy-paste template, the full field
reference, and the few formatting rules that matter.
